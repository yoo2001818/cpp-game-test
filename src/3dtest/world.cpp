...
std::shared_ptr<entity> world::add(entity &&pEntity) {
  if (mDeadEntityList.empty()) {
    std::shared_ptr<entity> entity_val =
        std::make_shared<entity>(std::move(pEntity));
...
void world::remove(const entity &target) { this->remove(target.getId()); }
...
std::shared_ptr<entity> world::get(const entity_id &id) const {
  ...
  if (!target->isAlive())
    return nullptr;
  if (target->getId().version != id.version)
    return nullptr;
  return std::move(target);
}

world::entity_iterator world::begin() {
  return world::entity_iterator(this->mEntityList.begin(),
                                this->mEntityList.end(), {});
}

world::entity_iterator world::end() {
  return world::entity_iterator(this->mEntityList.end(),
                                this->mEntityList.end(), {});
}

void world::markDirty(const entity &entity) { this->markDirty(entity.getId()); }
