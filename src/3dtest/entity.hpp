...
public:
  ...
  entity() : mComponentMap(), mEntityId(), mIsAlive(false), mIsDirty(false){};
  entity(entity &&pEntity)
      : mComponentMap(std::move(pEntity.mComponentMap)),
        mEntityId(pEntity.mEntityId), mIsAlive(pEntity.mIsAlive),
        mIsDirty(pEntity.mIsDirty){};
  entity(const entity &pEntity)
      : mComponentMap(pEntity.mComponentMap), mEntityId(pEntity.mEntityId),
        mIsAlive(pEntity.mIsAlive), mIsDirty(pEntity.mIsDirty){};
...
private:
  template <class T, class... Ts> std::tuple<T *, Ts *...> try_get_list() {
    if constexpr (sizeof...(Ts) == 0) {
      ...
      return std::tuple_cat(std::make_tuple(try_get<T>()),
                            try_get_list<Ts...>());
    }
  };
...
public:
  template <class T, class T2, class... Ts>
  std::tuple<T *, T2 *, Ts *...> try_get() {
    return try_get_list<T, T2, Ts...>();
  };
  template <class T> T &set(const T &value) {
    std::type_index type_index(typeid(T));
    auto [it, is_created] = this->mComponentMap.emplace(
        std::pair(type_index, std::make_any<T>(value)));
    if (!is_created) {
  ...
  const entity_id &getId() const { return this->mEntityId; };

  bool isAlive() const { return this->mIsAlive; }

  bool isDirty() const { return this->mIsDirty; }
